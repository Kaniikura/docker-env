...
3. **コンテナの起動**  
`sudo docker run -it -d -v $(pwd):/work/share -p <ポート番号>:8888 --rm --ipc=host --memory="32g" --memory-swap="-1" --cpus="8." --gpus="0,1" <イメージID> `
...
消し忘れると`docker container ls -a`を打ったときにお化けコンテナが大量に現れたりするので注意。
